...
| Commande                                                     | Utilité                  | Exemple IA                |             |
| ------------------------------------------------------------ | ------------------------ | ------------------------- | ----------- |
| `ps aux`                                                     | Tous les processus       | repérer `python train.py` |             |
| `ps -eo pid,ppid,user,pcpu,pmem,etime,cmd --sort=-pcpu`      | Vue triée CPU            | isoler consommateurs      |             |
| `ps --forest -o pid,ppid,cmd`                                | Arbre (ASCII)            | parents/enfants           |             |
| `pgrep -af python`                                           | PIDs + ligne de commande | retrouver entraînements   |             |
| `pidof python`                                               | PID d’un binaire         | automatisation            |             |
| `lsof -p <PID>`                                              | Fichiers ouverts         | logs/datasets utilisés    |             |
| `strace -p <PID>`                                            | Syscalls (diagnostic)    | blocages I/O              |             |
| \`watch -n 2 'ps -C python -o pid,pcpu,pmem,cmd --sort=-pcpu | head'\`                  | Refresh périodique        | suivi léger |
